...
const Dropdown = props => {
  ...
  const [richDataRenderer, setRichDataRenderer] = useState(
    props.renderComponent === undefined ? false : props.renderComponent
  )
  const [options, setOptions] = useState(props.options === undefined ? [] : props.options)
  const [selectedOptions, setSelectedOptions] = useState(
    options.filter(option => option.selected === true).map(({ value }) => value)
  )
  ...
  const documentClickHandler = () =>
    document.addEventListener('click', e => (dropdownRef.current.contains(e.target) ? false : setActive(false)))
    ...
    <DropdownContainer ref={dropdownRef}>
      ...
      <DropdownListContainer>
        <DropdownList active={active}>
          {props.search ? (
            <li>
              <DropdownSearchInput onChange={e => setSearch(e.target.value)} type="text" placeholder="Search" />
            </li>
          ) : (
            false
          )}
          {props.selectAllButton && options.length > 0 ? (
            <DropdownFilter onClick={() => setSelectedOptions(options.map(({ value }) => value))}>
              Select All
            </DropdownFilter>
          ) : (
            false
          )}
          {props.clearButton && options.length > 0 ? (
            <DropdownFilter onClick={() => setSelectedOptions([])}>Clear All</DropdownFilter>
          ) : (
            false
          )}
